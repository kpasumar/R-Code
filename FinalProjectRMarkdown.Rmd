...
table <- sort(table,decreasing = TRUE)   # Arrange variates by decreasing significance

sig_values <- which(table >= 0.05)      # Significant p-values from the sorted table
kable(table[,sig_values[1:9]],"latex", caption = "Variates/Interactions with significant p-values from F-test", 
      booktabs = T) %>%
kable_styling(latex_options = c("striped", "hold_position","scale_down"))
...
Assuming the insignificance threshold of 0.05, removing categorical/continuous variate interaction \texttt{bpmeds:prevstrk} esults in the following p-value.
...
# histogram
hist(res.manual, breaks = 50, freq = FALSE, cex.axis = cex,xlab = "Studentized Residual CHD Risk (Manual model)", main = "")
...
par(mfrow = c(1,2), mar = c(4,4,.1,.1))
# histogram
hist(res.step, breaks = 50, freq = FALSE, cex.axis = cex,xlab = "Studentized Residual CHD Risk (Stepwise model)", main = "")
