...
by(fhsd$chdrisk, fhsd$prevmi, summary)
```
...
pairs(~ chdrisk + prevmi, data = fhsd)
hist(fhsd$chdrisk)
# pair plots for continuous variables
pairs(~ chdrisk + totchol + age + sysbp + diabp + cigpday + bmi + 
...
library(stringr) # For string operations
 ...
 # Perform F-tests by removing one variate at a time 
...
sort(table,decreasing = TRUE)   # Look at the variates that show least significance

# Remove first three variates from table since starting the 4th variate, reduction becomes significant
anova(Mstep, update(Mstep,. ~ . - cigpday:heartrte - bpmeds:prevstrk - bpmeds:glucose)) 
```
