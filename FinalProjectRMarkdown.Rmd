...
par(mar = c(4,4,.5,.1))
plot(predict(Mstep), res.step, pch = 21, bg = "blue", cex = cex, cex.axis = cex,xlab = "Predicted logit(CHD Risk)", ylab = "Studentized Residuals")
abline(h = 0, lty = 1, col = "black") # add horizontal line at 0

# Then we analyze Mdl_manual
# get the hat values
h <- hatvalues(Mdl_manual)
res.manual <- resid(Mdl_manual)/sqrt(1-h) # studentized residuals, but on the data scale
cex <- .8 # controls the size of the points and labels
par(mar = c(4,4,.5,.1))
plot(predict(Mdl_manual), res.manual, pch = 21, bg = "red", cex = cex, cex.axis = cex,xlab = "Predicted logit(CHD Risk)", ylab = "Studentized Residuals")
abline(h = 0, lty = 1, col = "black") # add horizontal line at 0
```
